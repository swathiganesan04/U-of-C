...
# 1b) List the first name and last name of all the actors in the actor table.
SELECT 
  first_name, 
  last_name 
from 
...
ALTER TABLE 
  sakila.payment_type RENAME TO sakila.payment_types;

# 1i) Drop the table payment_types.
DROP TABLE IF EXISTS sakila.payment_types;
...
# 2g) What is the cost of renting the movie ACADEMY DINOSAUR for 12 days?  Alias the calculated column as rental_cost.
...
SELECT 
  ROUND(
    (rental_rate / rental_duration)* 12, 
    2
  ) AS rental_cost 
FROM 
  sakila.film 
...
FROM 
  sakila.rental 
WHERE 
  rental_date BETWEEN '2005-02-01 00:00:00' 
  AND '2005-06-01 00:00:00';
