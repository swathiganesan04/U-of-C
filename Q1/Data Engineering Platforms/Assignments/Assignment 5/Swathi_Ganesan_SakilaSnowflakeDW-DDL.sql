...
CREATE TABLE IF NOT EXISTS `sakila_snowflake`.`fact_rental` (
  ...
  CONSTRAINT `fact_rental_dim_customer_fk` FOREIGN KEY (`customer_key`) 
	REFERENCES `sakila_snowflake`.`dim_customer` (`customer_key`) 
	ON DELETE NO ACTION ON UPDATE NO ACTION, 
  CONSTRAINT `fact_rental_dim_staff_fk` FOREIGN KEY (`staff_key`) 
	REFERENCES `sakila_snowflake`.`dim_staff` (`staff_key`) 
	ON DELETE NO ACTION ON UPDATE NO ACTION, 
  CONSTRAINT `fact_rental_dim_film_fk` FOREIGN KEY (`film_key`) 
	REFERENCES `sakila_snowflake`.`dim_film` (`film_key`) 
    ON DELETE NO ACTION ON UPDATE NO ACTION, 
  CONSTRAINT `fact_rental_dim_store_fk` FOREIGN KEY (`store_key`) 
	REFERENCES `sakila_snowflake`.`dim_store` (`store_key`) 
    ON DELETE NO ACTION ON UPDATE NO ACTION, 
  CONSTRAINT `fact_rental_rental_date_fk` FOREIGN KEY (`rental_date_key`) 
	REFERENCES `sakila_snowflake`.`dim_date` (`date_key`) 
    ON DELETE NO ACTION ON UPDATE NO ACTION, 
  CONSTRAINT `fact_rental_return_date_fk` FOREIGN KEY (`return_date_key`) 
	REFERENCES `sakila_snowflake`.`dim_date` (`date_key`) 
    ON DELETE NO ACTION ON UPDATE NO ACTION
) 
ENGINE = InnoDB 
