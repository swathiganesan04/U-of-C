datapath <- "/Users/swathiganesan/Desktop/Q1/Statistical Analysis/Assignment 6/Individual/statistics_06_data"
...
LinearModel.Training<-read.table(paste(datapath,'Week6_Test_Sample_Train.csv',sep = '/'), 
                                 header=TRUE)
head(LinearModel.Training)
nSample.Training<-length(LinearModel.Training[,1])
head(LinearModel.Training)
plot(LinearModel.Training$Input,LinearModel.Training$Output, type="p",pch=19)
...
Classification.Rule.Logistic <- -coef(LinearModel.Training.Logistic)["(Intercept)"]/coef(LinearModel.Training.Logistic)["Logistic.Input"]
...
EstimatedLinearModel<-lm(main_dat$Output~main_dat$Input)
